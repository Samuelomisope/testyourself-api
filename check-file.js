...
async function main() {
  ...
  console.log('--- Count of legacy Wasabi vs R2 files ---');
  const counts = await client.query(
    `SELECT
       COUNT(*) FILTER (WHERE "fileUrl" ILIKE '%wasabisys%') AS wasabi_count,
       COUNT(*) FILTER (WHERE "fileUrl" ILIKE '%r2.cloudflarestorage%' OR "fileUrl" ILIKE '%${process.env.R2_BUCKET_NAME || ''}%') AS r2_count,
       COUNT(*) AS total
     FROM "StudyMaterial"`
  );
