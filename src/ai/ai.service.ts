...
export class AiService {
  ...
  private async extractPdfText(buffer: Buffer): Promise<string> {
    try {
      ...
      this.logger.error({ err }, 'PDF text extraction failed');
      return '';
    }
  }
  ...
  private async askClaude(
    ...
    try {
      ...
      this.logger.warn({ err }, 'Claude failed, falling back to Groq');
      return this.askGroq(
        system,
  ...
  private async ask(
    ...
    if (fileData && fileMimeType) {
      const isPdf =
        fileMimeType === 'application/pdf' || fileMimeType === 'pdf';
  ...
  async generateQuiz(
    ...
    const fileType = fileMimeType?.startsWith('video/')
      ? 'video'
      : isPdf
        ? 'PDF'
...
${text ? `Content:\n${text}` : ''}`;
    ...
    const raw = await this.ask(
      'You are UNILIB AI. Generate quiz questions STRICTLY from the provided document content only. Never generate generic study tips or meta questions. Return only raw JSON arrays with no markdown backticks, no explanation.',
      prompt,
      fileData,
      fileMimeType,
    );
  ...
  async generateQuizFromMaterial(
    ...
    if (!isPdf)
      throw new BadRequestException(
        'Only PDF materials are supported for quiz generation.',
      );
    ...
    if (!response.ok) {
      this.logger.warn(
        { materialId, status: response.status },
        'Failed to fetch study material file',
      );
    ...
    this.logger.debug(
      { materialId, bufferSize: buffer.length, extractedLength: extractedText.length },
      'Processed material for quiz generation',
    );
  ...
  async askQuestion(
    ...
    const answer = await this.ask(
      'You are UNILIB AI, a friendly and smart study assistant. Answer student questions clearly and concisely. Use simple language. If explaining a concept, give a short example. If a file is provided, analyze and explain it in detail.',
...
${text ? `Notes:\n${text}` : ''}`;
    ...
    const summary = await this.ask(
      'You are UNILIB AI, a study assistant. Summarize study material clearly and concisely for students. For bullet points, use • as the bullet character.',
      prompt,
      fileData,
      fileMimeType,
    );
...
${text ? `Text: ${text}` : ''}`;
    ...
    const raw = await this.ask(
      'You are UNILIB AI, a study assistant. Return only raw JSON arrays with no markdown backticks, no explanation.',
      prompt,
      fileData,
      fileMimeType,
    );
